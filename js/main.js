...
TAAG.setHash = function (page) {
  ...
  params.push(
    "f=" + encodeURIComponent(fontList.options[fontList.selectedIndex].text)
  );
...
TAAG.loadHash = function (hash) {
  ...
  if (page) {
    if (page === "display") {
      TAAG.updateFont(
        fontList.options[fontList.selectedIndex].value,
        function () {
          TAAG.updateDisplay();
          var txtBox = document.getElementById("inputText");
          txtBox.focus();
  ...
  if (isFiglet) {
    TAAG.lastAjax = $.ajax({
      ...
      success: function (res) {
        ...
        TAAG.currentFont.loadHorizontalOpts(
          charWidth.options[charWidth.selectedIndex].value
        );
        var charHeight = document.getElementById("taagCharHeight");
        TAAG.currentFont.loadVerticalOpts(
          charHeight.options[charHeight.selectedIndex].value
        );
...
TAAG.getText = function (txt, txtId, font, fontName) {
  ...
  fontName =
    fontName || fontList.options[fontList.selectedIndex].value.split(".")[0];

  var idStr =
    typeof txtId !== "undefined"
      ? "id='" + txtId + "'"
      : "id='taag_output_text'";
  return (
    "<pre " +
    idStr +
    " style='float:left;' class='" +
    TAAG.displayClass(font.getFontType(), fontName) +
...
TAAG.updateDisplay = function () {
  ...
  var txtBox = document.getElementById("inputText");
  var fontList = document.getElementById("fontList");
  document.getElementById("outputFigDisplay").className = TAAG.displayClass(
    TAAG.currentFont.getFontType(),
    fontList.options[fontList.selectedIndex].value.split(".")[0]
  );
...
TAAG.displayGeneratedImage = function () {
  ...
  tmp.style.cssText =
    "font-family: " +
    textDisplay.css("font-family") +
    "; font-size: " +
    textDisplay.css("font-size") +
  ...
  display.append(
    document.createTextNode(
      "You can download the image below by right clicking it as selecting 'Save Image As' (be sure to save it as a *.png file)"
    )
  );
  if (!$.browser.msie) {
...
TAAG.displayTestAll = function () {
  ...
  var output = document.getElementById("outputFigDisplay");
  var testAllHtml =
    '<div id=\'taagTestAll\' style="width:100%;"><pre style="text-align:center;" class="fig">' +
    TAAG.testAll.banner +
    "</pre>";

  testAllHtml +=
    "<div style=\"text-align:center;\" id='taagTestAllListLoaded'>Loaded: 0 of " +
    len +
    "</div><p/><div id='taagTestAllCancelDiv' style=\"text-align:center;\"><button id='taagTestAllCancel' onclick='javascript:TAAG.testAllCancel()'>Cancel</button></div><p/><div style=\"text-align:center;\">Sort order: Popular FIGlet fonts, Regular FIGlet fonts, AOL fonts</div><p/>";
  testAllHtml += "<p/><br/>";
  testAllHtml += "</div>";
    ...
    success: function (res) {
      try {
        ...
        if (isFiglet) {
          ...
          var charWidth = document.getElementById("taagCharWidth");
          TAAG.testAll.figlet.loadHorizontalOpts(
            charWidth.options[charWidth.selectedIndex].value
          );
          var charHeight = document.getElementById("taagCharHeight");
          TAAG.testAll.figlet.loadVerticalOpts(
            charHeight.options[charHeight.selectedIndex].value
          );
        ...
        elm.className = TAAG.displayClass(
          TAAG.testAll.font.getFontType(),
          fontName
        );
        ...
        divFontName.appendChild(
          document.createTextNode("Font Name: " + fontName)
        );
        divFontName.style.marginBottom = "4px";
        var btnUseFont = document.createElement("button");
        btnUseFont.style.marginRight = "8px";
        ...
        btnSelectAll.style.border = "2px solid green";

        btnSelectAll.appendChild(document.createTextNode("Copy TEXT GIRAFFE"));
        ...
        $(btnSelectAll).click(
          (function () {
            ...
            return function () {
              ...
              $(this).text("TEXT GIRAFFE copied :)");
              setTimeout(() => {
                $(this).css("background-color", "#3b3b3b");
                $(this).text("Copy TEXT GIRAFFE");
              }, 1500);
            };
          })()
        );
...
(function () {
  ...
  $("#inputText").bind("keyup", function (evt) {
    ...
    if (TAAG.fontReady()) {
      // don't run unless there is a font loaded
      TAAG.updateDisplay();
    }
  });
